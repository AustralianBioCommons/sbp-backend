...
@pytest.mark.asyncio
async def test_read_s3_bytes_success(mock_env_vars, mock_s3_client):
    ...
    result = await read_s3_bytes("results/test/file.tsv")
...
@pytest.mark.asyncio
async def test_read_s3_bytes_missing_bucket():
    """Test reading a text object without bucket configuration."""
    with patch.dict("os.environ", {}, clear=True):
        with pytest.raises(S3ConfigurationError, match="AWS_S3_BUCKET"):
            await read_s3_bytes("results/test/file.tsv")


@pytest.mark.asyncio
async def test_read_s3_bytes_client_error(mock_env_vars, mock_s3_client):
    """Test reading a text object wraps S3 client errors."""
    mock_s3_client.get_object.side_effect = ClientError(
        {"Error": {"Code": "NoSuchKey", "Message": "Not found"}},
        "get_object",
