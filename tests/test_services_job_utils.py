...
async def test_get_result_snapshot_downloads_returns_tracked_snapshots(test_db):
    ...
    snapshots = [
        S3Object(object_key=key, uri=f"s3://bucket/{key}")
    ...
    with (
        patch("app.services.job_utils.list_s3_files", new_callable=AsyncMock, return_value=[]),
        patch(
        ...
        result = await job_utils.get_result_snapshot_downloads(test_db, run)
...
async def test_get_result_snapshot_downloads_returns_empty_when_missing(test_db):
    ...
    with patch("app.services.job_utils.list_s3_files", new_callable=AsyncMock, return_value=[]):
        result = await job_utils.get_result_snapshot_downloads(test_db, run)
...
async def test_get_result_report_download_returns_tracked_report(test_db):
    ...
    with (
        ...
        result = await job_utils.get_result_report_download(test_db, run)
...
async def test_get_result_report_download_discovers_report_from_s3(test_db):
    ...
    report_key = (
        f"{run_id}/bindcraft/sampleF_0_output/Accepted/Animation/sampleF_report.html"
    )
...
async def test_get_result_report_download_falls_back_to_listing_when_sync_finds_nothing(test_db):
    ...
    with (
        patch("app.services.job_utils.sync_bindcraft_outputs", new=AsyncMock(return_value=[])),
        patch("app.services.job_utils._get_run_output_keys", return_value=[]),
        patch(
            "app.services.job_utils.list_s3_files",
            new_callable=AsyncMock,
            side_effect=lambda prefix: [{"key": report_key}] if prefix.endswith("Accepted/Animation/") else [],
        ),
        patch(
            "app.services.job_utils.generate_presigned_url",
            new_callable=AsyncMock,
...
async def test_get_result_snapshot_downloads_fall_back_to_listing_when_sync_finds_nothing(test_db):
    ...
    with (
        ...
        patch(
            "app.services.job_utils.list_s3_files",
            new_callable=AsyncMock,
            side_effect=lambda prefix: [{"key": snapshot_key}] if prefix.endswith("sampleH_0_output/") else [],
        ),
        patch(
            "app.services.job_utils.generate_presigned_url",
            new_callable=AsyncMock,
